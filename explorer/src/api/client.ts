...
function handleCypherErrors(response: CypherResponse): CypherResponse {
  if (response.errors && response.errors.length > 0) {
    const message = response.errors.map((e) => `${e.code}: ${e.message}`).join('\n')
...
export async function graphNeighborhood(
  ...
  return postJson<GraphPayload>(`/nornicdb/graph/${database}/neighborhood`, payload)
}
...
export async function graphTemporal(
  ...
  return postJson<GraphPayload>(`/nornicdb/graph/${database}/temporal`, payload)
}
